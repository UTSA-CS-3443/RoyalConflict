/*
 * The PlayController class controls the page that plays the game. This class controls a
 * button that leads back to the menu screen.
...
public class PlayController implements EventHandler < Event > , Initializable {
	...
	@FXML Button menuButton , play_again_button ;
	@FXML Label play_label , player_label , computer_label , player_deck_cards_left ,
	...
	@FXML ImageView player_deck , computer_deck , player_card , computer_card ,
	...
	public void play_again( Event event ) {
		...
		play_again_button.setDisable( true ) ; // Hide the play again button and disable
												// it
	...
	public void draw( Event event ) {
		if ( warOver == false ) {
			if ( warCardsLeft == 1 ) {
				...
				player_deck_cards_left.setText( "Cards: " + player.numberHandCards( ) ) ;
				// player_stack_cards_label.setText("Cards: " +
				...
				// computer_stack_cards_label.setText("Cards: " +
				...
				if ( player.numberHandCards( ) + player.numberStackCards( ) == 0 ) {
					...
					play_again_button.setDisable( false ) ; // Show the play again button
															// and enable it
					play_again_button.setVisible( true ) ;
				}
				else if ( computer.numberHandCards( )
				...
				if ( player_drawn_card.getRank( ) == computer_drawn_card.getRank( ) ) {
					...
					war_pool_label.setText(
								"Player won war pool: " + warPool.size( ) + " cards" ) ;
					...
					winning_card_label.setText(
								player_drawn_card.toString( ) + " is victorious!" ) ;
					// Update the info label
					...
					// Update war_pool_label
					war_pool_label.setText(
								"Computer won war pool: " + warPool.size( ) + " cards" ) ;
					warPool.clear( ) ; // Empty the war pool deck, someone has won them
					...
					computer_stack_cards_label
								.setText( "Cards: " + computer.numberStackCards( ) ) ;
					winning_card_label.setText(
								computer_drawn_card.toString( ) + " is victorious!" ) ;
					// Update the info label
					...
					if ( player.numberHandCards( ) + player.numberStackCards( ) == 0 ) {
						info_label.setText( "Player ran out of cards! Computer wins!" ) ;
						player_deck.setDisable( true ) ; // turn off deck button, game is
															// over
						play_again_button.setDisable( false ) ; // Show the play again
																// button and enable it
						play_again_button.setVisible( true ) ;
					}
					else if ( computer.numberHandCards( )
								+ computer.numberStackCards( ) == 0 ) {
				...
				// Add the cards to the war pool, so save them for the player who wins the
				// war
				warPool.add( player_drawn_card ) ;
				warPool.add( computer_drawn_card ) ;
			...
			if ( player.numberHandCards( ) + player.numberStackCards( ) == 0 ) {
				info_label.setText( "Player ran out of cards! Computer wins!" ) ;
				player_deck.setDisable( true ) ; // turn off deck button, game is over
				play_again_button.setDisable( false ) ; // Show the play again button and
														// enable it
				play_again_button.setVisible( true ) ;
			}
